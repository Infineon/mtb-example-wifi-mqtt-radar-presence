...
void mqtt_client_task(void *pvParameters)
{
    ...
    subscriber_data_t * subscriber_data = &subscriber_q_data;
    publisher_data_t * publisher_data = &publisher_q_data;
    ...
    if (pdPASS != xTaskCreate(subscriber_task, "Subscriber task", SUBSCRIBER_TASK_STACK_SIZE,
                              mqtt_connection, SUBSCRIBER_TASK_PRIORITY, &subscriber_task_handle))
    {
    ...
    if (pdPASS != xTaskCreate(publisher_task, "Publisher task", PUBLISHER_TASK_STACK_SIZE, 
                              mqtt_connection, PUBLISHER_TASK_PRIORITY, &publisher_task_handle))
    {
    ...
    /* Create the radar task and cleanup if the operation fails.  */
        ...
        if (pdTRUE == xQueueReceive(mqtt_task_q, &mqtt_status, portMAX_DELAY))
        {
            ...
            switch(mqtt_status)
            {
                ...
                case HANDLE_DISCONNECTION:
                {
                    ...
                    xQueueSend(subscriber_task_q, &subscriber_data, portMAX_DELAY);

                    /* Initialize Publisher post the reconnection. */
                    publisher_q_data.cmd = PUBLISHER_INIT;
                    xQueueSend(publisher_task_q, &publisher_data, portMAX_DELAY);
                    break;
                }
