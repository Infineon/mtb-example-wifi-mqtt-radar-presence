...
QueueHandle_t subscriber_msg_q;
...
void subscriber_task(void *pvParameters)
{
    ...
    mqtt_connection = (cy_mqtt_t ) pvParameters;
    ...
    {
        printf(" 'subscriber_task_q' queue creation failed... Task suspend\n\n");
    }

    /* Create a message queue to communicate with other tasks and callbacks. */
    subscriber_msg_q = xQueueCreate(SUBSCRIBER_TASK_QUEUE_LENGTH, sizeof(sub_msg));
    if (subscriber_msg_q == NULL)
    {
        printf(" 'subscriber_msg_q' queue creation failed... Task suspend\n\n");
    }
...
void mqtt_subscription_callback(cy_mqtt_publish_info_t *received_msg_info)
{
    ...
    if (strncmp(received_msg_info->topic, MQTT_SUB_TOPIC, sizeof(MQTT_SUB_TOPIC)-1) == 0) {
        if (xSemaphoreTake(sem_sub_payload, portMAX_DELAY) == pdTRUE)
        {
            ...
            if( xQueueSend(subscriber_msg_q, ( void * ) &sub_msg, portMAX_DELAY) != pdPASS)
            {
                printf("failed to post the message to subscriber_msg_q ");
            }
        }
    }
}
